...
def import_data(data="CIFAR"):
    if data == "CIFAR":
        Data = np.load('./datasets/CIFAR.npz')
    elif data == "Fash03":
        Data = np.load('./datasets/FashionMNIST0.3.npz')
    elif data == "Fash06":
        Data = np.load('./datasets/FashionMNIST0.6.npz')
...
def run_experiment(data="CIFAR", algorithm="bootstrap", output_path="./results/", est_t = False, runs=5, num_epochs_stage1=20,
    ...
    for run in range(runs):
        ...
        json_path = f"{output_path}/{data}/{algorithm}_run{run+1}.json"
    ...
    mean_T = np.mean(transition_matrix_estimates, axis=0)
    ...
    summary_path = f"{output_path}/{data}/{algorithm}_summary.json"

    summary_result = {
        'dataset': data,
        'algorithm': algorithm,
        'all_accs': [float(acc) for acc in all_test_accuracies],
        'mean_acc': float(mean_acc),
        'std_acc': float(std_acc),
        'T_estimate': [T.tolist() for T in transition_matrix_estimates],
...
def main(runs=3, output_path="./results/",datasets=["Fash03", "Fash06","CIFAR"], est_t = False):
    ...
    for result in results:
        print(f"\nDataset: {result['dataset']}, Algorithm: {result['algorithm']}")
...
if __name__ == "__main__":
    ...
    # You can customize which datasets to run
    # datasets = ["Fash03", "Fash06", "CIFAR"]
    datasets = ["Fash06"]

    main(runs=1, output_path = "./results/", datasets = datasets, est_t = True)